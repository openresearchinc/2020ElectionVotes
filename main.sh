...
for state in $states
do
  #Fetch streaming vote data from nyt.com in json format
  curl "https://static01.nyt.com/elections-assets/2020/data/api/2020-11-03/race-page/$state/president.json" |jq '.'> $state.json
  #Parse json for total #votes, multiply percentage for each candidate
  ...
  #timestamp trump-lost(+)  biden-lost(-) #lost for both in the same slot cancelling each other for better scale in gnuplot
